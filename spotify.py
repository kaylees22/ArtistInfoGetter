...
class Spotify:
    ...
    def get_artist_name(self, artist_id):
        response = requests.get(url=BASE_URL + f'artists/{artist_id}',headers = self.headers)
    ...
    def get_artist_releases(self, aid):
        """
            get 5 most recent releases from artist with given id
        """

        response = requests.get(url=BASE_URL + f'artists/{aid}/albums',headers = self.headers, params={'include_groups': 'album,single,appears_on'})
        data = response.json()
        
        sorted_releases = sorted(data['items'], key=lambda item: item['release_date'], reverse=True)[:5]
        releases = [{'name': item['name'], 'release date': item['release_date'], 'type': item['album_type']} for item in sorted_releases]
...
if __name__ == '__main__':
    ...
    print(spotify.get_artist_name('74KM79TiuVKeVCqs8QtB0B'))